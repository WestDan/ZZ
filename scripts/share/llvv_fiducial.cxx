...
// variables: l1,l2 (l1.pt > l2.pt)
//	      l3,l4 --> v1,v2 (v1.pt > v2.pt)
//	      z1,z2 --> z_ll, z_vv
//	      delta_R_ll_1,delta_R_ll_2 --> delta_R_ll,delta_R_vv

static double Z_MASS = 91.1876; // unit in GeV 
time_t start, finish;

void fid(string input_file, string output_file) 
{
    time(&start);
    ...
    {
	TLorentzVector lepton[2], neutrino[2];
	TLorentzVector jet[2];
	TLorentzVector z[2];
	for (int i=0; i<2; i++)
	{
	    lepton[i].SetPtEtaPhiE(0, 0, 0, 0);
	    neutrino[i].SetPtEtaPhiE(0, 0, 0, 0);
	...
	int index_l=0, index_v=0, index_j=0;
	for (int i=0; i<10; i++)
	{
	    ...
	    {
		lepton[index_l].SetPxPyPzE(px[i], py[i], pz[i], E[i]);
		index_l++;
	    }
	    else if ( abs(pdgID[i]) == 14 || abs(pdgID[i]) == 12 || abs(pdgID[i]) == 16) // neutrino
	    {
		neutrino[index_v].SetPxPyPzE(px[i], py[i], pz[i], E[i]);
		index_v++;
	    }
	}

	if ( index_l == 2 && index_v == 2 )
	{
	    z[0] = lepton[0] + lepton[1];
	    z[1] = neutrino[0] + neutrino[1];
	    delta_R_ll[0] = sqrt( pow( (lepton[0].Eta() - lepton[1].Eta()), 2) + pow( (lepton[0].Phi() - lepton[1].Phi()), 2) );
	    delta_R_ll[1] = sqrt( pow( (neutrino[0].Eta() - neutrino[1].Eta()), 2) + pow( (neutrino[0].Phi() - neutrino[1].Phi()), 2) );

	    // sort leptons
	    if(lepton[0].Pt() < lepton[1].Pt())
	    {
		TLorentzVector temp = lepton[0];
		lepton[0] = lepton[1];
		lepton[1] = temp;
	    }

	    /*
	     * don't need to sort neutrino
	    if(neutrino[0].Pt() < neutrino[1].Pt())
	    {
		TLorentzVector temp = neutrino[0];
		neutrino[0] = neutrino[1];
		neutrino[1] = temp;
	    }
	     */
	...
	l2_e   = lepton[1].E();
	l3_pt  = neutrino[0].Pt();
	l3_eta = neutrino[0].Eta();
	l3_phi = neutrino[0].Phi();
	l3_e   = neutrino[0].E();
	l4_pt  = neutrino[1].Pt();
	l4_eta = neutrino[1].Eta();
	l4_phi = neutrino[1].Phi();
	l4_e   = neutrino[1].E();
